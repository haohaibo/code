...
__global__ void sgemm(int M, int K, int N, float alpha, float beta, float* A, float* B, float* C)
{
    A += blockIdx.x * BM * K;
    B += blockIdx.y * BN;
    ...
    // use float4 vectorized global memory load
    // use float4 as one element
    int elementsLoadPerThreadA = ((BK * TM * TN) / BN) / 4;
    int elementsLoadPerThreadB = ((BK * TM * TN) / BM) / 4;
    ...
    // use float4 vectorized global memory load
    int colA = threadIdx.x % (BK/4);
    int rowA = threadIdx.x / (BK/4);

    int colB = threadIdx.x % (BN/4);
    int rowB = threadIdx.x / (BN/4);
    ...
    {
        for(int t = 0; t < elementsLoadPerThreadA; ++t)
        {
            float4 a = reinterpret_cast<float4*>(&A[((rowA * elementsLoadPerThreadA + t) * K) + colA * 4])[0];
            As[((4 * colA + 0) * BM) + (rowA * elementsLoadPerThreadA + t)] = a.x;
            As[((4 * colA + 1) * BM) + (rowA * elementsLoadPerThreadA + t)] = a.y;
            As[((4 * colA + 2) * BM) + (rowA * elementsLoadPerThreadA + t)] = a.z;
            As[((4 * colA + 3) * BM) + (rowA * elementsLoadPerThreadA + t)] = a.w;

            //As[(colA * BM) + (rowA * elementsLoadPerThread + t)] = A[((rowA * elementsLoadPerThread + t) * K) + colA];
        }

        for(int t = 0; t < elementsLoadPerThreadB; ++t)
        {
            //Bs[((rowB * elementsLoadPerThread + t) * BN) + colB] = B[((rowB * elementsLoadPerThread + t) * N) + colB];

            reinterpret_cast<float4*>(&Bs[((rowB * elementsLoadPerThreadB + t) * BN) + colB * 4])[0] = reinterpret_cast<float4*>(&B[((rowB * elementsLoadPerThreadB + t) * N) + colB * 4])[0];
        }
